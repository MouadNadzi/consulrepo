...
public class CarService {
    ...
    private final String URL = "http://service-client";  // Changed from localhost:8888
    ...
    private String getClientServiceUrl() {
        ...
        return "http://service-client"; // fallback URL
    }

    public List<CarResponse> findAll() {
        List<Car> cars = carRepository.findAll();
        ResponseEntity<Client[]> response = restTemplate.getForEntity(
                getClientServiceUrl() + "/api/client",
                Client[].class
        );
        Client[] clients = response.getBody();
        return cars.stream()
                .map((Car car) -> mapToCarResponse(car, clients))
                .toList();
    }
    ...
    public CarResponse findById(Long id) throws Exception {
        ...
        Client client = restTemplate.getForObject(
                getClientServiceUrl() + "/api/client/" + car.getClient_id(),
                Client.class
        );
        return CarResponse.builder()
                .id(car.getId())
...
public class CarService {
    ...
    @Autowired
    private RestTemplate restTemplate;
    private final String URL = "http://service-client";
    ...
    public CarResponse findById(Long id) throws Exception {
        Car car = carRepository.findById(id).orElseThrow(() -> new Exception("Invalid Car Id"));
        Client client = restTemplate.getForObject(
                this.URL + "/api/client/" + car.getClient_id(),
                Client.class
        );
        return CarResponse.builder()
                .id(car.getId())
